...
class HydroBlocks:
 ...
 def run(self,info):
  ...
  while date < self.fdate:
   ...
   if (date.hour == 0) and (date.day == 1):
    ...
    print(date.strftime("%Y-%m-%d"),'time:%10.4f'%(time.time()-tic),'et:%10.4f'%self.et,'prcp:%10.4f'%self.prcp,'q:%10.4f'%self.q,'WB ERR:%10.6f' % self.errwat,'ENG ERR:%10.6f' % self.erreng,flush=True)
 ...
 def create_netcdf_file(self,):
  ...
  metadata = {
             ...
             'qbase':{'description':'Excess runoff','units':'mm','dims':('time','hru',),'precision':4},
             'qsurface':{'description':'Surface runoff','units':'mm','dims':('time','hru',),'precision':4},
             'runoff':{'description':'Total runoff','units':'mm','dims':('time','hru',),'precision':4},
             'prcp':{'description':'Precipitation','units':'mm','dims':('time','hru',),'precision':4},
             'smc':{'description':'Soil water content','units':'m3/m3','dims':('time','hru','soil'),'precision':4},
             ...
             'smc1':{'description':'Soil water content at the top soil layer','units':'m3/m3','dims':('time','hru',),'precision':4},
             'relsmc1':{'description':'Relative soil water content at the top soil layer','units':'m3/m3','dims':('time','hru',),'precision':4},
             'smc_root':{'description':'Soil water content at the root zone','units':'m3/m3','dims':('time','hru',),'precision':4},  
