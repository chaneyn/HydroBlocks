...
class richards:
 ...
 def calculate_divergence_dense(self,h,T):
  ...
  with np.errstate(invalid='ignore',divide='ignore'):
   ...
   #w = np.array(self.width.todense()) # Moved to init
   #tmp = np.copy(w)
   #tmp = self.area/w
   #dx = (tmp + tmp.T)/2
   ...
   #area = self.area 
   #w = self.width_dense
   #dx = ( np.abs(self.dem[:,np.newaxis] - self.dem[np.newaxis,:])**2 + self.dx_w**2 )**0.5
   ...
   #calc_div = -1000.0*That*np.true_divide(dh,dx)*np.true_divide(np.copy(self.width_dense),area) # mm/s
   calc_div = -1000.0*That*np.true_divide(dh,self.dx_w)
