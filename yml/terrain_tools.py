...
#laura, function that allows basin clustering in 2-stages
def cluster_basins_hmc_2(basins,covariates,hp_in,nclusters,cluster_init):
 X = []
 for var in covariates:
  otmp = np.copy(covariates[var]['d'])
  otmp[(np.isnan(otmp) == 1) | (np.isinf(otmp) == 1)] = 0.0
  tmp = np.copy(otmp)
  #Standarize and apply weight, laura
  tmp = covariates[var]['w']*normalize_variable(tmp,covariates[var]['min'],covariates[var]['max'])
  X.append(tmp)
 X = np.array(X).T
 clusters = cluster_data(X,nclusters)+cluster_init
 basin_clusters = np.copy(basins)
 basin_clusters[:] = -9999

 count = 0
 for main in hp_in['bid']:
  basin_clusters[basins == main] = clusters[count]
  count += 1

 return (basin_clusters,)
