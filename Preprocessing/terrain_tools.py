...
def create_basin_tiles(basin_clusters,hand,basins,dh):
 ...
 for ubc in ubcs:
  ...
  for i in range(len(bin_edges)):
   if i == 0 : new_bin_edges.append(bin_edges[i])
   else: 
     if (bin_edges[i]-new_bin_edges[-1]) >= 1.0: new_bin_edges.append(bin_edges[i])
  bin_edges = np.asarray(new_bin_edges)
...
def create_hrus_hydroblocks(hillslopes,htiles,covariates,nclusters):
 ...
 varea = np.sum(hillslopes >= 0)
 # 277  grids ~ 0.25km2/hru 
 # 555  grids ~ 0.5 km2/hru
 # 1111 grids ~ 1.0 km2/hru
 # 2222 grids ~ 2.0 km2/hru
 # 3333 grids ~ 3.0 km2/hru
