...
def Prepare_Model_Input_Data(hydroblocks_info,metadata_file):
 ...
 if (hydroblocks_info['connection_matrix_hbands']==False):
  ...
  #for i in range(1,(int(hydroblocks_info['hmc_parameters']["number_of_characteristic_subbasins"]+1))): #luiz
  for i in range(1,(int(np.max(output['basin_clusters_map'])+1))): #luiz
   text='wmatrix_Basin%s' %int(i)
   #print('luiz print cid',cid,'i',i,'max ',np.max(output['basin_clusters_map']))
   #print('print cid',cid,'i',i,'size',output['cmatrix_Basin%s' %int(i)]['width'].data.size,output['cmatrix_Basin%s' %int(i)]['width'],flush=True)
...
def Compute_HRUs_Semidistributed_HMC(covariates,mask,hydroblocks_info,wbd,eares,input_dir):
 ...
 print('channels_wob',channels_wob,'mask',mask,'fdir',fdir,flush=True)
 print('unique channels',np.unique(channels_wob))
 basins_wob = terrain_tools.ttf.delineate_basins(channels_wob,mask,fdir)
 basins_999=np.copy(basins_wob)
 basins_999[basins_999 <= 0]=0

 #print(hydroblocks_info['cid'],'basins_wob',np.min(basins_999),np.max(basins_999),flush=True)
...
# flag_fd=hydroblocks_info['fully_distributed']
 ...
 if flag_fd==True: #laura
  ...
  #print('fully distributed',np.unique(basin_clusters),basins_wob,basins_wob.shape,flush=True)
 ...
 #Calculate average bankfull depth per basin cluster
  #print('basin_clusters',basin_clusters,'basins_wob',basins_wob,'cvs',cvs,'hp_in',hp_in,'ncatchments',ncatchments,flush=True)
 ...
 (db_routing,area_adj,new_hand2) = Build_Hillslope_River_Database(channels_wob,mask,fdir,eares,tiles,hand,basins_wob,basin_clusters,new_hand,db_routing,ubcs,tile_position,db_channels,hydroblocks_info['cid']) #luiz
...
#   lc_mask[covariates['lc_19']==1]=0.33 #moss/sedge/lichens, Alaska only
 ...
 #print(' CID',hydroblocks_info['cid'],'input hrus','basin',basin_clusters.shape,'tiles',tiles,'cvs',cvs,'nclusters',nclusters)
...
def Build_Hillslope_River_Database(channels_wob,mask,fdir,eares,tiles,hand,basins_wob,
    basin_clusters,new_hand,db_routing,ubcs,tile_position,db_channels,cid):
 ...
 for ubc in ubcs:
  ...
  #print('new_hand2',np.min(new_hand2[mnw]),np.mean(new_hand2[mnw1]),np.mean(db_channels['bankfull'][ubs-1],ubc,flush=True)
 #print('new_hand2',np.min(new_hand2),np.isnan(new_hand2),new_hand2.shape,flush=True)
 new_hand2[np.isnan(new_hand2) == 1] = 0.0
    
 #for i in range(0,tiles.shape[0]):
    #for j in range(0,tiles.shape[1]):
       #if tiles[i,j] == 203: print(cid,'tiles',tiles[i,j],basins_wob[i,j],new_hand2[i,j],i,j,flush=True)
 print(cid,'shapes',tiles.shape,basins_wob.shape,new_hand2.shape,flush=True)
 ...
 for i in range(basins_wob.shape[0]):
  for j in range(basins_wob.shape[1]):
   ...
   h = new_hand2[i,j]
   hband = tiles[i,j]
   if hband == 203: print(cid,'antes',hband,basin,h,i,j,flush=True)
   if basin <= 0:continue
   #if basin <= -1:continue
   ...
   print(cid,'db_routing',hband,h,tiles[i,j],np.min(db_routing['reach_hand_area'][basin][h]),np.min(db_routing['reach_hand_hband'][basin][h]),flush=True)
 ...
 for b in db_routing['reach_hand_area']:
  ...
  for ih in range(c_hand.size):
    ...
    #print('db_routing ih',np.min(c_area[ih]),np.max(c_area[ih]),ih,flush=True)
 ...
 for reach in db_routing['reach_hband_area']:
  for hband in db_routing['reach_hband_area'][reach]:
   ...
   reach2hband[reach-1,hband] = tmp
   #if tmp <= 0: 
   #reach2hband[reach-1,hband] = db_routing['reach_hband_area'][reach][hband]
   #print('reach2hband',reach2hband[reach-1,hband],reach-1,np.unique(list(db_routing['reach_hband_area'].keys())).size,hband,uhbands.size,flush=True)
   print(cid,'reach2hband',hband,reach2hband[reach-1,hband],reach-1,np.unique(list(db_routing['reach_hband_area'].keys())).size,uhbands.size,flush=True)
   #print('db_routing reach',np.min(reach2hband[reach-1,hband]),np.min(db_routing['reach_hband_area'][reach][hband]),hband,reach,np.unique(list(db_routing['reach_hband_area'].keys())).size,uhbands.size,flush=True)
   
 #print('zeros', np.argwhere(reach2hband == 0), len(np.argwhere(reach2hband == 0)))

 #Correct the area per grid cell array (and then apply to construct database)
 hband_areas = np.array(np.sum(reach2hband,axis=0))
 #hband_areas = np.sum(reach2hband * (reach2hband > 0), axis=0)
 for h in range(0,len(hband_areas)):
    print(cid,'hband_areas',np.min(hband_areas[h]),np.max(hband_areas[h]),h,hband_areas.shape,flush=True)
 area_adj = np.zeros(new_hand.shape)
 area_adj[:] = -9999.0
 for hband in uhbands:
  m = tiles == hband
  area_adj[m] = hband_areas[hband]/np.sum(m)
  print(cid,'area_adj',np.min(area_adj[m]),np.max(area_adj[m]),np.min(hband_areas[hband]),np.max(hband_areas[hband]),hband,np.sum(m),flush=True)
 #print('hband_areas',np.min(hband_areas),np.max(hband_areas),np.min(area_adj),np.max(area_adj),flush=True) #luiz
...
def Assign_Parameters_Semidistributed_svp(covariates,metadata,hydroblocks_info,OUTPUT,cluster_ids,mask,hbands,area_adj,dz_data,dz_model):
 ...
 for hru in np.arange(nhru):
  ...
  print(hydroblocks_info['cid'],hru, 'hband',OUTPUT['parameters']['hru']['hband'][hru],flush=True)
  #Calculate area per hru
  OUTPUT['parameters']['hru']['area'][hru] = np.sum(area_adj[idx])
  print(hydroblocks_info['cid'],hru,'area',np.min(OUTPUT['parameters']['hru']['area'][hru]),np.max(OUTPUT['parameters']['hru']['area'][hru]),flush=True)
...
#Removed covariates and cluster ids from parameters, replace nhrus for nhbands, laura
 ...
 cmatrix = sparse.coo_matrix((np.ones(hdst.size),(horg,hdst)),shape=(int(np.unique(hbands).shape[0]-1),int(np.unique(hbands).shape[0]-1)),dtype=np.float32) 
 # Retrieve the diagonal elements, laura
 diagonal_sparse = cmatrix.diagonal()
 #If hband is not connected to itself for more than 1 hband in the cluster of basins, add a single "fake" connection, laura
 if np.sum(diagonal_sparse==0)>=1:
  new_diagonal = np.copy(diagonal_sparse)
  new_diagonal[new_diagonal==0] = 1
  cmatrix.setdiag(new_diagonal) #laura, nhrus replaced with hbands
